...
6. **连续处理**：对话框保持打开，切换到下一张图像后再次点击 **Apply** 即可；点击 **Cancel** 关闭对话框

### 使用 New Instance（新实例）

对话框左下角的 **New Instance** 按钮（蓝色三角图标）可创建处理实例（process icon）：

- 点击后对话框会关闭，图标出现在工作区
- 将图标**拖到图像上**即可用该参数处理图像
- 或将图标**拖到工作区保存**，方便以后重复使用

> ⚠️ 注意：由于 PixInsight 限制，脚本实例不能在脚本运行时递归执行，因此点击 New Instance 后对话框会自动关闭。若需连续处理多张图像，请使用 **Apply** 按钮。
