...
4. 在弹出的对话框中设置反卷积参数，点击**执行**
5. 等待处理完成，结果会在新窗口中打开
6. **连续处理**：处理完成后对话框会再次弹出，切换到下一张图像后再次点击"执行"即可；点击"取消"结束脚本

> 💡 脚本会保持运行，无需为每张图片重新打开脚本。对话框标题会显示当前处理的图像名称。
...
**Q: 处理完一张图片后需要重新打开脚本吗？**
A: 不需要。脚本会保持运行并重复弹出参数对话框，切换到下一张图像后再次点击"执行"即可连续处理。点击"取消"结束脚本。
